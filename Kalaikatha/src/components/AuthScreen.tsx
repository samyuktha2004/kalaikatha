...
export function AuthScreen({ onSuccess, initialUserType = 'buyer' }: AuthScreenProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (mode === 'login') {
        ...
        await signup(name, identifier, password, userType);
      }
      onSuccess();
    } catch (error) {
  ...
  const VoiceButton = ({ field }: { field: 'name' | 'phone' }) => (
    <button
      type="button"
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    required
                    className={`${INPUT_BASE_CLASS} ${userType === 'artisan' ? 'pr-12' : ''} ${theme.ring}`}
                    placeholder="Enter your name"
                  />
                  {userType === 'artisan' && <VoiceButton field="name" />}
                </div>
                {userType === 'artisan' && (
                  <p className="text-xs text-gray-500 mt-1 flex items-center gap-1">
                    <Mic className="w-3 h-3" /> Tap mic to speak your name
                  </p>
                )}
              </div>
            )}
              ...
              {loginMethod === 'email' ? (
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    />
                    {userType === 'artisan' && mode === 'signup' && <VoiceButton field="phone" />}
                  </div>
