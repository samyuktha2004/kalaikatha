...
import { ArrowLeft, Upload, Sparkles, AlertCircle, Check, Loader, Image as ImageIcon } from 'lucide-react';
import { useState, useRef } from 'react';
import { useFileUpload, useImageAnalysis, useDeviceCapability } from '../../hooks/useArtisanFeatures';
...
export function AIStudio({ onBack }: AIStudioProps) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-purple-50 via-pink-50 to-orange-50 dark:from-gray-900 dark:via-gray-800 dark:to-gray-900 p-4 pt-20">
          ...
          <div>
            ...
            {showDetailedText && (
              <p className="text-gray-600 dark:text-gray-400 text-sm mt-1">
                📸 Upload photos → AI enhances & protects trade secrets
              </p>
            )}
          </div>
        </motion.div>
            ...
            <div className="grid md:grid-cols-2 divide-x divide-gray-200 dark:divide-gray-700">
              ...
              <div className="p-6 md:p-8">
                <div className="flex items-center justify-between mb-4">
                  <h3 className="text-lg text-gray-900 dark:text-white">Your Photo</h3>
                  <button
                    onClick={handleUploadClick}
                ...
                {analyzing && (
                  <div className="mt-4 flex items-center gap-3 text-indigo-600 dark:text-indigo-400">
                    ...
                    <span className="text-sm">Azure AI analyzing image...</span>
                  </div>
                )}
              </div>
              ...
              <div className="p-6 md:p-8 bg-gray-50 dark:bg-gray-900">
                <h3 className="text-lg text-gray-900 dark:text-white mb-4">AI Analysis</h3>

                {!analysis && !analyzing && (
                  <div className="text-gray-500 dark:text-gray-400 text-sm">
                    Waiting for Azure AI analysis...
                  </div>
                )}
                  ...
                  <div className="space-y-4">
                    ...
                    {analysis.tradeSecrets.length > 0 && (
                      <div className="bg-amber-100 dark:bg-amber-900/30 rounded-xl p-4">
                        ...
                        <ul className="text-sm text-amber-800 dark:text-amber-400 space-y-1">
                          {analysis.tradeSecrets.map((secret: any, idx: number) => (
                            <li key={idx}>• {secret.type}: {secret.reason}</li>
                          ))}
                        </ul>
                        <p className="text-xs text-amber-700 dark:text-amber-500 mt-2">
                          These will be saved to your Protected Vault only
                        </p>
                      </div>
                    )}
                    ...
                    {analysis.objects.length > 0 && (
                      <div>
                        ...
                        <div className="flex flex-wrap gap-2">
                          {analysis.objects.slice(0, 6).map((obj: any, idx: number) => (
                            <span
                              key={idx}
                              className="px-3 py-1 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-full text-xs text-gray-700 dark:text-gray-300"
                            >
                    ...
                    {analysis.tags.length > 0 && (
                      <div>
                        ...
                        <div className="flex flex-wrap gap-2">
                          {analysis.tags.slice(0, 8).map((tag: string, idx: number) => (
                            <span
                              key={idx}
                              className="px-3 py-1 bg-indigo-100 dark:bg-indigo-900/30 text-indigo-700 dark:text-indigo-400 rounded-full text-xs"
                            >
                              #{tag}
                            </span>
                          ))}
        ...
        {uploadedUrl && analysis && (
          ...
          >
            <button
              onClick={() => {
                // Navigate to vault
                alert('Saved to Protected Vault! 🔒');
              }}
            ...
            <button
              onClick={() => {
                ...
                alert('Generating marketing content... ✨');
              }}
